...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 2.1 | **LLM client (worker-only)** + structured JSON + grounding prompt with untrusted-content delimiters. **Pluggable provider**: OCI GenAI (OpenAI-compatible, `llm_openai.py`, Grok) / Gemini (`llm.py`) / stub — config-selected. `[tdd:required]` | Test (mocked): schema-valid JSON; delimiters + grounding present; key never logged. **OCI validated live e2e** (real Grok answer, $0.0017/call). | 1.2 | cc:完了 (OCI + Gemini + stub; gemini-flash escalation still TODO) |
...
| 2.4 | **Quote verification + grounding guards**: every returned quote verified as substring of cited review; validate ids; `not_enough_data` path. Retry→escalate on schema failure still TODO. `[tdd:required]` | Test: fabricated quote dropped/flagged; unknown id rejected; thin-evidence → `not_enough_data:true` | 2.1 | cc:完了 (verify_quotes + per-side verify; one repair retry on OCI + Gemini paths, tested; planner-model escalation deferred — needs spend-policy decision first) |
...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 3.1 | **Auth**: per-user identity via Google OAuth sessions (+ local dev fallback, anon trial). No API keys are issued and no magic-link flow exists — deliberately session-only for first launch, so there is nothing to hash/revoke; revisit if programmatic access is needed. `[tdd:required]` | Test: unauth request rejected; identity attached to jobs | 1.1 | cc:完了 (auth.py sessions + callback; no-key posture recorded here) |
| 3.2 | **Per-user quota + rate limiting** (per identity), enforced on cache-miss. `[tdd:required]` | Test: quota decrements on cache-miss; 429 past quota; rate-limit per identity not raw IP | 3.1, 2.5 | cc:完了 (combined analyze+compare quota on miss; per-identity in-process buckets; per-IP edge limit deferred to Cloudflare/Caddy) |
| 3.3 | **Global spend kill-switch + provider cap**: atomic persisted daily $ counter checked before every Gemini call; disables analysis at cap; alerts 50/80/100%; document Gemini project hard cap + Cloud budget alerts. `[tdd:required]` | Test: simulated spend ≥ cap → analysis disabled + alert fired; counter survives restart (DB-backed); provider cap documented | 2.5 | cc:完了 (atomic DailySpend reserve on both LLM paths; 50/80/100% log alerts tested; provider-console budgets remain an owner-box step per DEPLOY) |
| 3.4 | **Security hardening**: prompt-injection boundary (delimiting, no tools/secrets, escaped plain-text rendering); `app_id`/keyword validation (anti-SSRF); generic error handler (no key/stack leak); CORS, security headers, body-size limits, prod `/docs` gated. `[tdd:required]` | Tests: injection-laced review/question can't alter output or leak prompt; bad `app_id` rejected; errors generic; headers present | 2.1, 1.2 | cc:完了 (delimiter strip + verify + clamp; app_id validation; generic 500/502 codes; explicit same-origin CORS; 1MB body cap; docs/secret guards — all tested) |
| 3.5 | **Privacy/retention**: forced ingest anonymization verified; TTL purge for reviews + user Q/A; data-subject delete path; privacy policy text. `[tdd:required]` | Test: purge job removes expired rows; delete-my-data path works; privacy policy present | 1.1, 0.3 | cc:完了 (retention.py hourly purge + DELETE /api/me/data + served /privacy; test_retention/test_security cover all three) |
...
| 4.3 | **Sign-in UX + quota chip + error/empty states**: OAuth sign-in screen (magic-link deferred with 3.1); "N/M left today" chip; error states for app-not-found/scrape-blocked/LLM-error shipped in 4.1. `[tdd:required]` | All listed states render a friendly message; quota chip decrements | 4.1, 3.1, 3.2 | cc:完了 (SignInUpsell + header quota chip + friendly error states live) |
...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 5.1 | **CI/CD + hosting**: Dockerfile + Docker Compose (web + worker + Postgres) on a single box, TLS via bundled Caddy; GitHub Actions (ruff/format/mypy/pytest gate) + image boot-smoke; single-box first launch (no staging split, no render.yaml — revisit at scale); Alembic before first populated-DB change. `[tdd:skip:deploy-config]` | Push to main → green CI → image boots; deploy runbook in DEPLOY.md | 1.2 | cc:done (compose+Caddy+CI+boot smoke verified locally; TLS/Postgres proof needs the box) |
| 5.2 | **Cost monitoring + smoke test**: in-app 50/80/100% log alerts (done, tested); provider budget alerts + project hard cap (owner console); CI image boot-smoke (done); periodic scraper-breakage canary on the box. `[tdd:skip:ops-config]` | In-app alerts + CI smoke live; provider evidence + periodic canary are owner-box steps | 5.1, 3.3 | cc:TODO (code done; needs provider screenshots + box cron canary) |
| 5.3 | **Legal/docs**: LICENSE; hosted disclaimer (owner bears scraping/ToS risk); privacy policy; README rewrite (what it is, how to use). `[tdd:skip:docs-only]` | LICENSE + disclaimer + privacy policy live; README reflects the service | - | cc:完了 (MIT LICENSE + owner-risk disclaimer in Readme + served /privacy + stale-cache serving w/ caveat) |
| 5.4 | **Invite-only beta launch**: tight quotas, waitlist gate. `[tdd:skip:launch]` | Service reachable behind HTTPS, invite-gated, quotas live | 5.1, 5.2, 5.3, Phase 3 | cc:done (SIGNUP_OPEN gate in OAuth callback + DEPLOY beta section; flip to 0 on the box at launch) |
...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 6.1 | **API enrichment**: surface app `icon` + `histogram` in search/result; add shareable analysis permalink (`GET /api/analysis/{id}`). `[tdd:required]` | Tests: icon+histogram in payloads; permalink returns a cached analysis | 2.5 | cc:完了 |
| 6.2 | **Google OAuth + sessions** (Authlib): `User` table, `/auth/login|callback|logout`, `/api/me`. Auth-optional locally (no creds → local dev user), required in prod. `[tdd:required]` | Protected routes 401 w/o session when OAuth set; local fallback works; landing+health+permalink public | 1.1 | cc:完了 (auth.py; live OAuth pending owner creds) |
...
| 6.4 | **Real OCI cost**: record `cost_in_usd_ticks` from the response into `usage_log` (replaces estimate). `[tdd:required]` | Test: usage row cost matches OCI-reported ticks | 2.1 | cc:完了 (llm_openai ticks→cost_usd + worker prefers real cost in usage_log; test_llm_openai covers ticks) |
...
| 6.7 | **Owner setup (handoff)**: Google OAuth client + redirect URI; subdomain DNS → Ampere IP; Sarvam spend limit/alert. `[tdd:skip:owner-config]` | Documented checklist; values supplied to deploy | - | cc:done 2026-09-26 (DataBuoy OAuth creds live on box; reviews.topfolio.in DNS+TLS live; deployed bb44abe; first real Sarvam compare done on prod; Sarvam console spend guard still owner TODO) |

---

## Phase 7: Monetization & Competitive Intelligence Engine

| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 7.1 | **Dodo self-serve billing & paid tier quotas**: `User` tier/dodo fields, `billing.py` checkout & webhook handling, `/api/billing/checkout` & `/api/billing/webhook`, quota tier scaling (50 for starter, 200 for pro, +20 for pass). `[tdd:required]` | Unit tests in `test_billing.py` pass; mock activate works without secrets | 6.2 | cc:完了 (Stripe path removed; Dodo-only) |
| 7.2 | **Battle Lens (Competitor Comparison Engine)**: Multi-app comparative schema, comparative prompt, and multi-corpus quote verification. `[tdd:required]` | Architectural spec complete; API contract and prompt isolation designed | 2.1, 1.3 | cc:完了 (POST /api/compare + status/permalink routes, compare worker w/ spend cap + stub fallback, comparison prompt/schema, per-side verify, /compare + /c/:token UI; test_compare/test_analysis/test_security cover) |
| 7.3 | **1-Click Executive Exports**: Markdown/Notion export in result header; PDF export engine. `[tdd:skip:frontend]` | Markdown copy button live on results screen | 6.5 | cc:完了 |
| 7.4 | **Outreach Campaign & 10-Lead Intelligence Deck**: Actionable lead sheet for 10 high-value apps across 5 categories with customized Trojan Horse scripts. `[tdd:skip:docs-only]` | 10 pre-audited leads and ready-to-send messages documented | - | cc:完了 |
